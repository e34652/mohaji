...
CREATE TABLE `reg_course` (
	...
	`sub_id`	int	NOT NULL	COMMENT 'PK/auto',
	`cc_id`	varchar(30)	NULL	COMMENT 'FK',
	`rc_stat`	varchar(15)	NULL	COMMENT '수강중/이수/수강취소',
	`rc_progress`	decimal(4,1)	NOT NULL	DEFAULT 0,
	`rc_asgn_score`	int	NULL,
	`rc_att_score`	int	NULL,
	`rc_grade`	varchar(6)	NULL
);
...
CREATE TABLE `reg_detail` (
	...
	`rd_creason`	varchar(100)	NULL	COMMENT '(휴학, 과목변경, 기타개인사유)'
);
...
CREATE TABLE `reg_session` (
	...
	`rs_final`	int	NOT NULL	DEFAULT 0
);
...
CREATE TABLE `session` (
	...
	`session_title`	varchar(90)	NOT NULL,
...
CREATE TABLE `reg_asgn` (
	...
	`ra_content`	varchar(4000)	NULL,
	`ra_sdate`	timestamp	NOT NULL,
	`ra_score`	int	NULL,
	`ra_feedback`	varchar(4000)	NULL	COMMENT '기능추가시'
);
...
ALTER TABLE `reg_course` ADD CONSTRAINT `FK_students_TO_reg_course_1` FOREIGN KEY (
	`stu_id`
)
REFERENCES `students` (
	`student_id`
...
ALTER TABLE `reg_asgn` ADD CONSTRAINT `FK_students_TO_reg_asgn_1` FOREIGN KEY (
	`stu_id`
)
REFERENCES `students` (
	`student_id`
