    ...
    SecurityFilterChain defaultSecurityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests((requests) -> requests // HTTP 요청을 인증 및 권한 부여합니다.
                        .requestMatchers("/static/**", "/css/**", "/img/**" ,"/lib/**", "/js/**", "https://lms.ssu.ac.kr/**").permitAll() // "모든 외부링크 적용 허용"
                        .requestMatchers("/", "/main").permitAll() // "/"와 "/main" 경로에 대한 요청은 모두 허용합니다.
                        .requestMatchers("/register").permitAll() // "/register" 경로에 대한 요청은 모두 허용합니다.
                        .requestMatchers("/authenticated", "/notification/**").authenticated() // "/authenticated"와 "/notification/**" 경로에 대한 요청은 인증이 필요합니다.
